...
import { toCurl, stripJsonComments } from "@/lib/curl";
...
export function RequestEditor({
  ...
  const saveExample = () => {
    ...
    const updatedRequest = {
      ...request,
      examples: [...(request.examples || []), newExample],
    };
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border/50 bg-card/30 backdrop-blur-sm sticky top-0 z-10">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="GET" className="text-emerald-400">
              GET
            </SelectItem>
            <SelectItem value="POST" className="text-blue-400">
              POST
            </SelectItem>
            <SelectItem value="PUT" className="text-amber-400">
              PUT
            </SelectItem>
            <SelectItem value="DELETE" className="text-red-400">
              DELETE
            </SelectItem>
            <SelectItem value="PATCH" className="text-purple-400">
              PATCH
            </SelectItem>
          </SelectContent>
        </Select>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded">
                    JSON · comments stripped on send
                  </span>
                </div>
                <div className="code-editor-container border border-border/50 rounded-lg overflow-hidden bg-[#1a1d2e]">
                  <Editor
                    value={request.body || ""}
              ...
              {response ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-muted-foreground">
                    Hit <span className="text-primary font-medium">"Send"</span>{" "}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {request.examples?.map((ex, idx) => (
                    <div
                      key={ex.id}
                      className="border border-border/50 rounded-xl p-4 space-y-4 bg-card/30"
                    >
                      <div className="flex justify-between items-center gap-3">
                        <Input
                          value={ex.name}
                          onChange={(e) =>
                            updateExample(ex.id, "name", e.target.value)
                          }
                        ...
                        <Button
                          ...
                          onClick={() =>
                            updateField(
                              "examples",
                              request.examples.filter((e) => e.id !== ex.id),
                            )
                          }
                        >
                        ...
                        >
                          <Editor
                            ...
                            onValueChange={(val) => {
                              ...
                              updateExample(ex.id, "responseBody", parsed);
                            }}
                            highlight={(code) => {
                              try {
                                return Prism.highlight(
                  ...
                  {(!request.examples || request.examples.length === 0) && (
                    <div className="text-center py-16 border border-dashed border-border/50 rounded-xl">
                      ...
                      <p className="text-xs text-muted-foreground/60 mt-1">
                        Save responses to document your API
                      </p>
                    </div>
                  )}
                </div>
...
function ParamsEditor({
  ...
  const handleValuePaste = (id: string, pastedText: string) => {
    if (type !== "header") {
      handleUpdate(id, "value", pastedText);
      return;
    }
    ...
    if (match && match[1] && match[2]) {
      const newParams = params.map((p) =>
        p.id === id
          ? { ...p, key: match[1].trim(), value: match[2].trim() }
          : p,
      );
      onChange(newParams);
    } else {
  ...
  return (
    <div className="space-y-2">
      ...
      {params.map((p) => (
        <div
          key={p.id}
          className="grid grid-cols-[32px_1fr_1fr_1fr_40px] gap-3 items-center group"
        >
          <div className="flex justify-center">
            <Checkbox
              checked={p.enabled}
              onCheckedChange={(c: boolean | "indeterminate") =>
                handleUpdate(p.id, "enabled", c === true)
              }
          ...
          <Input
            ...
            placeholder="Key"
            className={cn(
              "h-9 text-sm font-mono bg-input/50",
          ...
          <Input
            ...
            onPaste={(e) => {
              ...
              handleValuePaste(p.id, pastedText);
            }}
            placeholder="Value"
            className={cn(
              "h-9 text-sm bg-input/50",
              !p.enabled && "opacity-50",
            )}
          />
          <Input
      ...
      <Button
        ...
        size="sm"
        onClick={handleAdd}
        className="mt-3 border-dashed border-border/50 text-muted-foreground hover:text-foreground"
      >
...
function ResponseShapeEditor({
  schema,
  onChange,
}: {
  schema: ResponseField[];
  onChange: (s: ResponseField[]) => void;
}) {
  const handleAdd = () => {
    onChange([
      ...schema,
  ...
  return (
    <div className="space-y-4">
      ...
      {schema.map((field) => (
        <div
          key={field.id}
          className="grid grid-cols-[1fr_100px_80px_1fr_40px] gap-2 items-start"
        >
          <Input
            value={field.name}
          ...
          <Select
            value={field.type}
            onValueChange={(v) => handleUpdate(field.id, "type", v)}
          >
          ...
          <div className="flex items-center justify-center h-8">
            <Checkbox
              ...
              onCheckedChange={(c: boolean | "indeterminate") =>
                handleUpdate(field.id, "required", c === true)
              }
            />
          </div>
          <Input
            ...
            onChange={(e) =>
              handleUpdate(field.id, "description", e.target.value)
            }
            placeholder="Description"
      ...
      <p className="text-xs text-muted-foreground mt-4">
        * Nested object support not implemented in MVP editor UI, but structure
        supports it.
      </p>
    </div>
  );
